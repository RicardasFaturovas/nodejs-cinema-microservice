const repository = (connection) => {
  ...
  const getCinemasByCity = (cityId) => {
    return new Promise((resolve, reject) => {
      ...
      const sendCinemas = (err) => {
        if (err) {
          reject(new Error(`Ann error occurred fetching cinemas, err: ${err}`));
        }
        resolve(cinemas);
      };
  ...
  const getCinemaById = (cinemaId) => {
    return new Promise((resolve, reject) => {
      ...
      const projection = {_id: 1, cinemaPremieres: 1};
      const response = (err, cinema) => {
        if (err) {
          reject(new Error(`Ann error occurred retrieving a cinema, err: ${err}`));
        }
        resolve(cinema);
      };
  ...
  const getCinemaScheduleByMovie = (options) => {
    return new Promise((resolve, reject) => {
      ...
      const sendSchedules = (err, result) => {
        if (err) {
          reject(new Error('An error has occurred fetching schedules by movie, err: ' + err));
        }
        resolve(result);
      }
